...
namespace EMRView
{
    ...
    public class emrMSDB
    {
        ...
        public const string Sql_GetInchRecordList = "SELECT rec.ID, cdes.pid AS despid, cdes2.name AS despname, rec.desID, cdes.od AS desorder,   cdes.Name AS desName, rec.Name, rec.CreateUserID, rec.CreateDT, rec.LastUserID, rec.LastDT   FROM Inch_RecordInfo rec LEFT JOIN Comm_DataElementSet cdes ON rec.desID = cdes.id    LEFT JOIN (SELECT id, name, od FROM Comm_DataElementSet WHERE pid = 0) AS cdes2 ON cdes2.id = cdes.pid     WHERE PatID = {0} AND VisitID = {1} ORDER BY cdes2.od";
